...
import com.example.liyayu.myapplication.util.LogD
...
class IrregularButton : View {
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        try {
            ...
            LogD("widthCaps == $widthCaps caps == $caps  stoke == $stoke  mTextSize == $mTextSize ")
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        LogD("onMeasure")
        LogD("measuredWidth == $measuredWidth  measuredHeight ==$measuredHeight   width = $width height == $height measuredWidthAndState == $measuredWidthAndState measuredHeightAndState = $measuredHeightAndState")
...
//根据控件宽度和缝隙大小计算按钮上下梯形宽度
        ...
        LogD("totalWidth == $totalWidth 控件高度 totalHeight = $totalHeight " +
                "longWidth == $longWidth shortWidth == $shortWidth  caps = $caps 梯形宽度差 widthCaps= $widthCaps" +
                "sharpeStokeWidth = $sharpeStokeWidth wideStokeWidth = $wideStokeWidth 直角长边为 l = $l  ")
    }
        ...
        if (canSelect) {// 是否可以做选择操作
            ...
            when (event.action) {
                MotionEvent.ACTION_DOWN -> {
                    ...
                    if (isLeftContains) {
                        selectListener?.onClick(true)
                    }
                    if (isRightContains) {
                        selectListener?.onClick(false)
                    }
